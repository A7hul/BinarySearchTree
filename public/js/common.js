...
function find_node(find_tree_node) {
  const input = document.querySelector(".node-key");
  document.querySelector(".find").onclick = ((el) => {
    const val = input.value;
    const v = parseInt(val, 10);
    if (!isNaN(v) && 0 <= v && v <= NODE_MAX_KEY) {
      find_tree_node(v);
...
function find_left_or_right_most_node(find_l_r_tree_node) {
  document.querySelector(".submit").onclick = ((el) => {
    ...
    if (direction==null) {
      ...
      message.innerHTML = 'Select radio button to click Submut.';
    }else{
      if (!isNaN(v) && 0 <= v && v <= NODE_MAX_KEY) {
        find_l_r_tree_node(v,direction)
      }
    }
  });
}
